...
**Authorship is read, not inferred, and the distinction is sharper than it looks.** A rule's branch may contain a *planner*, and everything that planner goes on to propose is a model's work: it is replannable, by that planner, and the ladder above does not apply to it. Walking ancestry to find a router cannot tell the two apart, because the router is an ancestor of both — a live run stalled at L4 on work no rule had ever seen. The engine therefore reads the `source` recorded on the `subgraph/proposed` that froze the vertex, which the router executor stamps on its own proposals ([10](./10-deterministic-routers.md)). Only the branch the rule itself authored is barred from L1 and L2.
...
**Everything is one unit: money.** Context tokens, expected planning output, registered tool calls, and compensation actions are priced before addition because adding unlike raw quantities is dimensionally meaningless. A currency estimate is also reviewable: a human can sanity-check "this replan is estimated at CNY 0.8," while an untyped score has no operational interpretation.

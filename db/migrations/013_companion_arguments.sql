-- A companion operation is called with the arguments its own tool declared.
--
-- The Coordinator used to send a confirm or a cancel the arguments of the *try*
-- it was closing. That is an inference, and a wrong one: a companion takes the
-- identity of what was reserved, not the parameters the reservation was made
-- with, so its input schema is usually narrower and the gateway refuses the
-- call. On the confirm path that refusal lands after the pivot has passed,
-- where the scope can no longer be rolled back, so the transaction can neither
-- commit nor cancel.
--
-- The mapping is now declared by the tool that owns the bracket and resolved by
-- the engine at freeze, so what arrives here is already the companion's own
-- arguments. Nothing in the database or the Coordinator constructs them.
ALTER TABLE txn_bracket ADD COLUMN IF NOT EXISTS confirm_input JSONB;
ALTER TABLE txn_bracket ADD COLUMN IF NOT EXISTS cancel_input JSONB;
...
BEGIN
    ...
    ELSIF NEW.event_type = 'txn/try' THEN
        ...
        VALUES (bracket_key, NEW.run_id, NEW.scope_id, 'sealed', (NEW.payload->>'deadline_at')::TIMESTAMPTZ, NEW.vertex_id, NEW.run_seq,
                ...
                -- Frozen by the engine from the mapping the tool declared. Stored beside the try's
                -- own arguments rather than derived from them: they are different arguments to a
                -- different tool, and the only reason they ever looked interchangeable is that
                -- nobody was carrying the companion's.
        ...
        IF cancel_phase = 'requested' THEN
            ...
            WITH RECURSIVE members AS (
                SELECT b.try_vertex_id AS vertex_id, b.idempotency_key, COALESCE(b.cancel_tool, b.compensate_tool) AS inverse_tool,
                       -- The inverse tool's arguments, not the try's. A rollback that sends the
                       -- try's arguments is refused at dispatch exactly like a confirm is.

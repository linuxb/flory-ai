...
| PostgreSQL schema and migrations | `db/`; the TypeScript suites provision and use `<database>_test` through `test/setup/database.ts` | [Doc 08](doc/design/08-database-schema.md) |
...
- Writes counterfactual fork output to a live business stream, reads `is_counterfactual` rows in a production fold or report, or feeds a run business context by re-folding an entity at assembly time instead of through a pinned snapshot in `task_input`. See [Doc 01 §5.2](doc/design/01-jit-dag-and-event-log.md#52-fork-is-a-bounded-causal-counterfactual-on-an-immutable-history) and [Doc 04 §2.1](doc/design/04-refine-and-harness-state.md#21-business-context-enters-through-task_input-not-harness-state).

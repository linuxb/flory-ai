...
### Watching a run recover

`npm run demo:retail` takes an optional fault, so the run hits a real failure and the recovery
ladder has something to do:

```bash
FLORY_DEMO_FAULT=inventory.check npm run demo:retail
```

The named tool then fails every attempt, scheduled through the sandbox's own fault table, so the
engine learns about it exactly as it would learn about an outage. The run shadows the discarded
subtree, calls the chosen planner again with the failure as evidence, and continues in the same
run — and the console draws what was abandoned beside what replaced it.

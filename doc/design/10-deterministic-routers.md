...
**Step 4 — deterministic branch failure.** Branch 2 executed, and the reservation fails permanently because the inventory service is down. The Coordinator cancels `S2` back to `R1`'s savepoint. Backtracking to `P1` is illegal below the backtrack floor, and replanning through `P2` is forbidden because the failed work came from a deterministic policy rather than a plan. The run halts at L4 with a full causal postmortem; no funds moved twice and no inventory is stranded.

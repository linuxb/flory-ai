...
- Leaves a tool-caller vertex with a planner as a direct successor, lets a planner emit router syntax, admits a router branch without exhaustive freeze-time checking, lets a router declare or infer its own transaction scope, dereferences a blob or reads a clock during router evaluation, or renders a fall-through router into a planner prompt. See [Doc 10](doc/design/10-deterministic-routers.md) and [Doc 02 §3.4](doc/design/02-transaction-model.md#34-deterministic-check-rules).

...
| Before pivot; retries exhausted, every member attempt resolved | Fence the scope, run one scope-level cancel over all sealed TCC and completed Saga members, return to the savepoint, and replan — or, when the failed work came from a deterministic router branch, halt to L4 without replanning ([03 §2.5](./03-replan-and-recovery.md#25-deterministic-branches-never-replan-with-a-model)). |

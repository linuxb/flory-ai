...
- compensation and confirmation tool references where the selected mode requires them, each with the **argument mapping** that says how to call it: a companion parameter name against a path into this tool's own arguments. A confirm acts on the identity of what was reserved rather than on the parameters the reservation was made with, so its input schema is usually narrower, and only the tool that owns the bracket knows which of its arguments carry that identity. Nothing downstream may infer it: an executor that forwards the try's own arguments is refused at dispatch, and on the confirm path that refusal arrives after the pivot, where the scope can no longer be rolled back ([02 §4.1](./02-transaction-model.md#41-runtime-lifecycle));
...
| G9 | A declared companion carries an argument mapping, and every source in it is a path into this tool's own declared arguments. A mapping without its companion, and a companion without its mapping, are both refused |

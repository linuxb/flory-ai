...
`console/server/src/projection/` holds a versioned fold beside the canonical surface projection. It is a projection in the same sense: a pure function of one run's events in `run_seq` order, reproducible from the log, carrying its own version (`console-projector@v2` — v2 added the ladder's cancellation requests as `cancel_requests`) so a rendering can be attributed.
...
**Every edge carries an arrowhead, and selecting a vertex highlights its immediate family.** Both exist for one reason: a canvas that shows two vertices are related without showing which derived which answers the wrong question. In a graph a planner grew one frozen chunk at a time, *what came from what* is the question. A selection colours the vertices that derived it and the vertices it derived in two invariant hues, dims everything else to a sixth of its opacity, and shows a legend naming the two directions.

Direct relations only. An ancestor highlighted the same way as a parent would silently answer a different question, so tracing a lineage is clicking along it and every step stays exact.

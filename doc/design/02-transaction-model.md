...
The same "derive, never ask" principle fixes where a **router** sits relative to a transaction. A router has no side effects and is never a scope member, so its placement is computed from the ancestor graph at freeze time: `at_savepoint` when every ancestor scope is closed, and `inside_scope(S)` when an ancestor scope `S` still holds a sealed but unconfirmed try. A branch emitted at a savepoint opens a fresh scope; a branch emitted inside `S` joins it. R12 and R13 (§3.4) admit each case, and [10 §4](./10-deterministic-routers.md#4-placement-and-transaction-binding) specifies the join shapes — a pre-declared common scope for atomic parallel branches, and read-only-only admission where parallel branches hold disjoint open scopes. There is no runtime scope-merge protocol: atomicity across parallel branches is declared at freeze time or it does not exist.
...
|---|---|---|
| **Prohibition** — X must not exist | **Yes.** Monotone: a violation appears at the moment of addition and cannot be repaired by later additions | R1, R2, R3, R7, R10, R12, R13, R14 |
...
| R12 | **Placement and scope invariants for a router branch.** *Inside `S`:* a branch must not declare `opens_scope`, because an open scope is not a savepoint and starting an independent scope inside a half-open try violates §4.1. A branch with a pivot is rejected when `S` already holds one (R3, across freezes) and admitted when `S` holds none, supplying `S`'s unique commit point. *At a savepoint:* a branch may declare `opens_scope`, and any branch with `max_effect ≠ none` must declare a scope or receive the engine-computed minimum (R10). | A deterministic branch is admitted against the transaction shape it will actually land in, not against the template in the abstract. |
| R13 | An `at_savepoint` router must have no unclosed ancestor scope; every tool its branches reference must exist in the run's role-scoped tool view; and every condition field path must statically resolve against upstream tool output schemas. | Moves configuration defects — an unauthorized tool, a renamed field — to a point where no pivot has passed and the planner can still replan. |
| R14 | **Router topology invariant.** No tool-caller vertex may have a planner vertex as a direct successor; every such edge carries a router, interposed by the engine when the proposal does not supply one. | Keeps the planner's action vocabulary free of router syntax while guaranteeing business policy an interception point at every thought junction, and keeps DAG topology invariant under rule attachment ([10 §2.1](./10-deterministic-routers.md#21-two-entry-tracks)). |

The current implementation boundary is deliberately split:

1. `ToolRegistry.validate()` validates the immutable tool view and enforces registration obligations R4 and R6 before proposal admission. Rule templates carry their own Engine-side publication gate, Q1–Q6 ([10 §3.4](./10-deterministic-routers.md#34-publication-admission-q1q6)).
2. `checkSubDag(proposal, toolView, existingScopeSnapshot)` computes ancestry, descendants, scope membership, pivots, conflicting write footprints, and confirmation-barrier coverage, then enforces proposal rules R1-R3 and R5-R14. Its third argument is an immutable snapshot of current scope states: R12 cannot decide whether a branch may open a scope or supply a pivot without knowing whether an ancestor scope is half-open and whether it already holds one.
...
4. A freeze that introduces a router runs `checkSubDag` **exhaustively over every branch of its pinned template against every reachable placement**, not only over the branch that will eventually match ([10 §5.1](./10-deterministic-routers.md#51-freeze-time-exhaustive-admission)). Static admission bounds shape only. Runtime admission still re-checks scope state atomically and may reject a branch that was shape-valid at freeze, so freeze admission does not promise zero runtime rejection.
...
| v0.4 | **R12, R13, and R14 added for deterministic routers; `checkSubDag` gained an `existing_scope_snapshot` argument.** A router emits sub-DAGs without a model call, so its branches need the same admission a planner's proposal receives — but at freeze time and for *every* branch, because the alternative is discovering an illegal shape after an irreversible pivot has passed and the proposing planner has fallen below the backtrack floor. R14 additionally makes interposition mandatory, which is what keeps DAG topology invariant under rule attachment. Two alternatives were rejected on entry: (a) *letting a router infer its own scope at runtime* — a vertex with no side effects cannot make transaction-structure judgements, and runtime inference bypasses the widening channel of §3.2; (b) *forcing upstream scopes closed at a router join to manufacture a savepoint* — committing a try early destroys reservation semantics and prematurely raises the backtrack floor. |
...
**A companion is called with the arguments its own tool declared.** A confirm or a cancel takes the identity of what was reserved, not the parameters the reservation was made with, so its input schema is usually narrower than the try's. Forwarding the try's arguments is therefore not a harmless default but a call the gateway refuses, and on the confirm path that refusal arrives after the pivot, where the scope can neither commit nor roll back. The mapping from companion parameter to a path into the try's arguments is declared on the try's own contract ([09 §3](./09-tool-registry-gateway.md#3-registration-and-the-tool-view-contract), rule G9), resolved by the engine at freeze, and recorded in `txn/try` alongside the try's own arguments. The Coordinator sends what was frozen and constructs nothing, so a replay issues the identical companion call.

Resolving at freeze rather than at execution is what makes that last property hold: a companion built at execution time would read a contract that may since have been republished, and the replay would then confirm with arguments the original run never sent. It also bounds what a mapping may read — the try's frozen arguments, not its output, which is not yet known when the graph freezes.

If any pre-pivot member fails, the scope is **fenced** in the same transaction that records the failure: it hands out no more work and admits no pivot. A fence has no external effect and is not a cancellation. Whether to cancel is decided by the Engine, which decides the replan the cancellation serves: it appends `replan/cancel-requested` naming the scope ([03 §2.3](./03-replan-and-recovery.md#23-replan-flow)), and the Coordinator, which only executes, picks the request up, appends `txn/cancel` with phase `requested`, and cancels every sealed TCC member and compensates every completed Saga member in reverse dependency order. `txn/cancel` is a scope action, never a per-try action. Its `completed` phase is appended only after the entire scope has returned to its savepoint, and nothing before `completed` counts as the bracket being closed.

The Coordinator used to cancel on its own the moment a member failed, and the recovery ladder read the log without waiting for it. Two parties deciding *when* independently was two races: a ladder reading before the cancellation began escalated a failure that a moment later had a legal boundary, and one reading between `requested` and `completed` replanned across a try whose inverse had not run. The database now refuses `txn/cancel {requested}` unless the Engine asked for it or a sealed try has expired (§4.4), so there is one initiator per cause.
...
| Before pivot; retries exhausted, every member attempt resolved | The failure fences the scope. The Engine requests one scope-level cancel over all sealed TCC and completed Saga members; the Coordinator runs it back to the savepoint; the Engine then replans — or, when the failed work came from a deterministic router branch, halts to L4 without replanning ([03 §2.5](./03-replan-and-recovery.md#25-deterministic-branches-never-replan-with-a-model)). |
| Before pivot; a member attempt is unresolved | **Do not cancel.** A recorded start with no recorded outcome may still produce an effect. Preserve the attempt evidence and the reservations, suspend the scope, and escalate to L4 (§4.4).                     |
...
**In a live run**, one sweep handles both entry and recovery. It is the one cancellation the Coordinator still starts by itself — an orphaned try that no failure and no replan will ever answer. It first finds `open` scopes containing a sealed bracket whose `try_timeout_s` deadline has elapsed, then atomically requests one idempotent scope cancellation. It uses the scope's one cancel key, the same as an Engine request, so the two converge on a single cancellation whichever arrives first. It also finds `cancelling` scopes for which no incomplete cancellation member has an effective `lease_until > now()` and resumes their remaining inverse operations. This includes a scope with no remaining member work, so a crash immediately before the terminal `txn/cancel {phase: completed}` append is recoverable. A live member lease prevents takeover; an expired lease is permission to retry the same idempotent inverse operation. No correctness-critical timer or cancellation cursor exists only in memory.

**A lease expiry is not evidence.** A lease grants a worker temporary ownership; its expiry proves only that the worker stopped renewing. It does not prove that the external request stopped, and it does not prove that no effect occurred. Three disciplines follow, and the runtime that implements them is [07 §3.4](./07-distributed-transaction-coordinator.md#34-orphan-sweeper).

1. **Durable attempt evidence.** Before sending a side-effecting request, the Coordinator durably records the attempt identity, idempotency key, and start, while validating scope state and lease ownership under the scope lock. The attempt stays **unresolved** until a definitive outcome is recorded. Queue deletion, lease expiry, and transport timeout never erase or resolve that evidence, and a recorded start with no recorded outcome is treated as unresolved even when the worker may have crashed before sending.
2. **Expiry means reconcile or suspend.** Under the scope lock, the sweeper rechecks the expired sealed try (a candidate whose try no longer qualifies is `ineligible`), the scope state, live leases, and unresolved attempts. A live lease defers cancellation. An unresolved attempt preserves the queue and the evidence, records suspension, and escalates to L4; suspension keeps pivot-admission and pivot-passage evidence and never clears the no-cancel fence. The registered pivot status query remains the one recovery that may establish occurrence or absence before deciding. For an unresolved **non-pivot** attempt there is no automatic reconciliation protocol: an operator establishes the final external outcome and ensures the original request can no longer create an effect before authorizing recovery. A transient "not found" is insufficient while the original request may still arrive, and a late worker result is evidence only — it authorizes no new dispatch, no automatic unsuspension, and no omission of cleanup.
3. **Only a clean scope cancels automatically.** An `open` scope with an expired sealed try, no live execution lease, and no unresolved side-effecting attempt is the sole configuration that may enter cancellation without an Engine request. The Coordinator then atomically appends `txn/cancel {phase: requested}` and removes pending ordinary work, retaining attempt history and every recorded effect that cancellation needs. Post-pivot states never take this path.

This is deliberately conservative: it trades automatic cleanup for explicit intervention whenever an external outcome is unknown, and it buys that without adding downstream fencing tokens. The failure it refuses to accept is a lease timer being mistaken for proof that cancellation is safe.

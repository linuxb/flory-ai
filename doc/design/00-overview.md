...
| event log | The append-only truth, in two planes: `run_event_log` for what a run orchestrated, `business_event_stream` for what a business entity experienced. Sole source of truth for the DAG *and* for transaction brackets. | [01 §3.1](./01-jit-dag-and-event-log.md#31-two-planes-and-three-sequences), [08](./08-database-schema.md) |
| `run_seq` | Position within one run's orchestration events. Strict, contiguous, rollback-safe, commit-ordered; the only legal input to a surface fold and the anchor of every intra-run reference. | [01 §3.3](./01-jit-dag-and-event-log.md) |
| business stream | One aggregate root's event sequence — `order:12345` — ordered by `stream_seq` across every run of its lifetime. The only legal input to a semantic fold, and what makes cross-run continuity a query rather than a correlation exercise. | [01 §3.1](./01-jit-dag-and-event-log.md#31-two-planes-and-three-sequences) |
| `global_seq` | A coarse `BIGSERIAL` over the physical table: gappy, not commit-ordered, never a fold input. Replication, CDC, and operational triage only. | [01 §3.3](./01-jit-dag-and-event-log.md) |
| business-state snapshot | One business stream folded to a pinned `stream_seq` and referenced from `task_input`, so a later run consumes entity history without the prompt drifting as the entity advances. | [04 §2.1](./04-refine-and-harness-state.md#21-business-context-enters-through-task_input-not-harness-state) |
...
| check-rules | Deterministic static validation before a DAG is frozen: transaction properties such as idempotent retry after a pivot (R1–R11), and router placement and topology (R12–R14). | [02 §3.4](./02-transaction-model.md#34-deterministic-check-rules), [10](./10-deterministic-routers.md) |
| `gatewayd` Tool Registry Gateway | The boundary that authenticates callers, publishes immutable role-scoped tool views, and routes one execution attempt without owning transaction policy: MCP to the executors, gRPC to tool services. | [09](./09-tool-registry-gateway.md) |
...
| fork | Offline historical counterfactual: branch a new run at any vertex, substitute one or more `pin_version`s, invalidate the vertex's causal descendants, lazily merge causally independent events, then compare folded surfaces up to `eval_up_to_seq` — for that case only. | [01 §5.2](./01-jit-dag-and-event-log.md), [05 §3](./05-context-aggregation-and-offline-evaluation.md) |
...
The current deployment view is [diagram/deployment-architecture.html](../diagram/deployment-architecture.html). It shows the Agent Orchestrator, Distributed Transaction Coordinator, `gatewayd`, PostgreSQL with both event planes, blob storage, business services, and the read-only Console, including registration, role-binding, and execution-routing paths. The self-contained diagram adapts to light and dark themes and provides a Light/Dark/Auto control.

The [animated architecture overview](../animations/architecture-dark.gif) in the README walks the same components through one transaction scope, from planning to the pivot and its confirmations; its generator is [doc/animations/src](../animations/src). The [conceptual architecture overview](../diagram/architecture.html) remains a higher-level companion. Focused mechanism diagrams are editable Draw.io files: [transaction boundaries](../diagram/txn-boundary.drawio), [replanning flow](../diagram/replan-flow.drawio), [projection and offline evaluation](../diagram/projection.drawio), and [Coordinator/Engine interaction](../diagram/coordinator-engine-interaction.drawio). [Router interposition, template binding, and sub-DAG admission](../diagram/router-admission.html) is a self-contained HTML companion to [10](./10-deterministic-routers.md), with the same Light/Dark/Auto control. The operator-facing view of a running graph is specified in [11](./11-console-and-observability.md) and mocked up in [ui-mockups/flory_dag_console.jpg](../ui-mockups/flory_dag_console.jpg).

### 3.1 Service and language boundaries

Flory is divided into four components over one database, because its planner, transaction runtime, tool boundary, and operator view have different correctness and operating constraints:
...
- The **Go 1.25 Distributed Transaction Coordinator** owns transaction brackets, timeout sweeping, orphan recovery, retry scheduling, effectful tool execution, the execution of the scope cancellations the Engine requests, and adapters that belong in existing Go infrastructure. [07](./07-distributed-transaction-coordinator.md) specifies its runtime boundary.
- The **Go 1.25 Tool Registry Gateway** is a separate service and module. It authenticates callers, publishes immutable role-scoped tool views, and routes one pinned attempt without taking transaction authority. [09](./09-tool-registry-gateway.md) specifies that boundary.
- **PostgreSQL** holds both event planes, harness metadata, business-state snapshots, synchronous transaction projections, work queues, and the gateway's role bindings, and allocates per-run and per-entity write order.
...
- **The Console is a read-only process and a renderer.** It is the repository's third standing service, after the Coordinator and `gatewayd`, and its first production Node listener: a small TypeScript HTTP server holding a versioned observability projection of the same log, and a browser client that draws what it is handed ([11](./11-console-and-observability.md)). It folds no events, derives no scope structure, and runs no graph algorithm in the browser; a browser-side fold would be the second projector the rule above exists to forbid.
- **The Console reads as its own database role.** `console_role` holds `SELECT` on `run_event_log` and nothing else, so "the Console writes nothing" is enforced by PostgreSQL rather than by review. Its HTTP surface is `GET`-only for the same reason, and it binds to loopback unless explicitly overridden, because who may read a run is still open ([11 §7](./11-console-and-observability.md#7-open-questions)).
...
- **Folding events in the browser** to spare the Engine an observability projection is rejected for the same reason, and with a worse failure mode: the disagreement would reach an operator as a picture of a graph that never existed ([11 §6](./11-console-and-observability.md#6-rejected-alternatives)).

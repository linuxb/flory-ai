...
| S2 | carrier permanently refuses, pre-pivot | The failure fences its scope; the engine appends `replan/cancel-requested`; the Coordinator cancels; only after `txn/cancel {completed}` does the engine append the L1 `replan/boundary`, citing the scope in `cancelled_scopes`. Asserted over every interleaving of the two parties by the model test `engine/test/unit/recovery-cancel-interleavings.test.ts` and against real rows at every split point by `engine/test/integration/recovery-cancellation.test.ts` | O2.cancel_before_replan, O2.cancel_request_discipline, O3 | T-B |
| S2b | replan is triggered while a `txn/try` is still open — before the cancellation was requested, or between `requested` and `completed`; the engine is offered a boundary at a planner inside the bracket | **negative test**: `replan/boundary` must not be appended with a `boundary_seq` inside an open bracket, and no escalation may be recorded while the failure's scope could still cancel. Cancel precedes replan (03 §2.4 rule 1); the database refuses a shadow into an unclosed scope; peak holds never exceed demand. The model test runs the pre-change ladder as a negative control and requires both defects to be found | O1.no_amplification, O2.cancel_before_replan, O2.one_answer_per_failure | T-B |
...
| S3c | two planners alternate: the replan at P2 produces a subgraph whose failure backtracks to P3, whose replan would otherwise return to P2. No single planner reaches `N` consecutive failures | **oscillation**: the per-planner counter never trips. Permit at most `E = 2` replanning `replan/boundary` events in one failure episode regardless of planner identity; on the next failure, request the failure's scope cancellation and then record L3 as an escalation (`selected: null`), never a third replan. The log records the `P2 -> P3` prefix, the failure that would recur at P2, and the L3 outcome. `E = 2` is derived from the S1 TLC lasso in 03 §6. | O3, O5.detour | T-B |
| S3b | no pivot has passed in the run, so the backtrack floor is the run start. The failure's nearest ancestor planner P2 sits **inside** an open `txn/scope` bracket whose savepoint precedes P2 | P2 fails the bracket condition (03 §2.1 (i)). Cancelling returns the world to a savepoint before P2, so P2's premises are undone: escalate to **L2** and select the **nearest** legal boundary at or after both the floor and the cancelled scope's savepoint, here P1 (03 §2.2 step 1). P2 stays `savepoint_precedes` after the cancellation has run, not only before it | O2, O3 | T-B |
...
| S11 | Coordinator restarts after a successful try has sealed a bracket but before the scope reaches pivot or cancellation | after `try_timeout_s`, the sweeper — the one cancellation the Coordinator starts without an Engine request — fences the open scope and idempotently cancels every sealed member ([02 §4.4](./02-transaction-model.md#44-orphan-try-detection)) | O1, O2 | T-B runtime integration |
...
| S16 | **deterministic branch failure**: a router matches, its branch's TCC try fails permanently pre-pivot, every member attempt resolved | the engine requests the scope's cancellation, the scope cancels to the savepoint, then **L4**: the only `replan/boundary` after the failure is that escalation, with `selected: null`, and no planner is called anywhere after it; a model call or a replan that selects a planner here is a failure even if the run would have recovered ([03 §2.5](./03-replan-and-recovery.md#25-deterministic-branches-never-replan-with-a-model)) | O2.no_deterministic_replan, O3 | T-B |
| S17 | **prompt invisibility**: one scenario run twice, once with no template bound to the slot and once with a template bound whose conditions all evaluate false | both runs reach the downstream planner with an **identical prompt hash, compared after renaming each vertex to its rank in causal order**. The normalization is not a weakening: two runs allocate fresh identifiers for every vertex and a prompt is ordered by identifier ([01 §4.2](./01-jit-dag-and-event-log.md#42-linearization)), so no two runs of any scenario are ever literally byte-identical and an unnormalized comparison would detect nothing. Everything the row is about — which items appear, in which relative order, with which contents — is compared verbatim. Topology is identical in both runs because interposition is mandatory; a rendered fall-through line would change every junction of every prompt | O4.router_invisibility | T-B |
...
| S19 | **cancel-versus-claim race**: a cancellation (an Engine request being picked up, or the sweep) and a worker claim contend for the same scope | decisive in both directions and never both: if cancellation commits first, no member adapter call starts; if the claim commits first, the live lease defers cancellation, and an Engine request stays pending until it can be decided | O1, O2 | T-B runtime integration |
...
| S24 | **the same log, read two ways**: one run in which a bound template's conditions all evaluate false, folded by both projections | the fall-through router is **on the console canvas** and **absent from the downstream planner's prompt**, in the same run. S17 proves invisibility by comparing two runs; this proves the conjunction inside one, which is what the console's reason for existing actually claims — two projections of one log disagreeing, deliberately, about the same vertex | O4.console_router_visibility | T-B |
...
| **rule-pin substitution**: a fork substituting only a router's bound rule inherits a structurally identical prefix — the same roles with the same parent structure — and differs from its source in `pin_version` alone | 10 §3.3 |
...
| **router invisibility**: for a scenario pair differing only in whether a non-matching template is bound, the downstream planner's prompt hash is identical once vertices are renamed to their causal rank (S17) | a rendered fall-through would appear at every junction, since R14 interposes a router on every tool-caller-to-planner edge, and would destroy prefix stability on every turn |
| **console router visibility**: in one run, the fall-through router appears in the console projection and is absent from the downstream planner's `linearize` output (S24) | the two projections are supposed to disagree here, and only an assertion inside a single run can say that the *same* vertex is both drawn and unprompted. Two-run comparison (S17) cannot: it shows a prompt is unchanged, not that anything is visible anywhere |

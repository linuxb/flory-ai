...
| `replan/boundary` | In-place replan: selected `boundary_seq`, planner vertex, reason, cancelled scopes and the request they came from. With `selected: null` it is an escalation, recorded only once every cancellation the failure needed has resolved. No new run is created (§5.1) | engine |
| `replan/cancel-requested` | The recovery ladder asks for the scope cancellations a replan or escalation needs: the failed vertex, the scopes, the intended boundary, and the candidate set as it stood before cancelling. The only way a failure-driven cancellation starts; the Coordinator executes it ([03 §2.3](./03-replan-and-recovery.md#23-replan-flow)) | engine |
...
### 5.2 Fork is a bounded causal counterfactual on an immutable history
...
4. **A Bounded Horizon, Fixed at Creation.** A counterfactual does not run to completion. Its horizon is one target sequence from the source run, `eval_up_to_seq`, chosen when it is created, and the engine executes the new causal chain and inherits independent events up to exactly that point. Everything it will ever inherit is copied in the creating transaction: the horizon cannot be extended afterwards, so there is nothing a later copy could learn, and a copy arriving later would arrive *below* the position a reader following the log had already passed — an inherited row keeps its source `run_seq`, at or under `eval_up_to_seq`, while the counterfactual's own events are numbered above it. This produces a comparable fold.

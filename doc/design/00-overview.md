...
- **The Console is a renderer, not a component with state.** It holds no database, folds no events, and runs no graph algorithm; the Engine serves it a versioned observability projection and it draws what it is handed ([11](./11-console-and-observability.md)). A browser-side fold would be the second projector that the rule above exists to forbid.

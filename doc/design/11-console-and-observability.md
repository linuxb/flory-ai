...
- **The shadowed-branch rendering has no producer yet.** Nothing appends `subgraph/shadowed` or `replan/boundary`, because online replanning is not implemented. The projection handles shadowing correctly and is unit-tested against hand-built events; the feature lights up when the recovery loop lands, and until then the Console's headline feature shows nothing in a live run.

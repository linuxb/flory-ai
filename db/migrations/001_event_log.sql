...
CREATE TABLE run_event_log_p0 PARTITION OF run_event_log FOR VALUES WITH (MODULUS 4, REMAINDER 0);
CREATE TABLE run_event_log_p1 PARTITION OF run_event_log FOR VALUES WITH (MODULUS 4, REMAINDER 1);
CREATE TABLE run_event_log_p2 PARTITION OF run_event_log FOR VALUES WITH (MODULUS 4, REMAINDER 2);
CREATE TABLE run_event_log_p3 PARTITION OF run_event_log FOR VALUES WITH (MODULUS 4, REMAINDER 3);
CREATE INDEX run_event_log_run_type_idx ON run_event_log (run_id, event_type, run_seq);
CREATE INDEX run_event_log_scope_idx ON run_event_log (run_id, scope_id, run_seq) WHERE scope_id IS NOT NULL;
...
BEGIN
    ...
    IF NEW.event_type IN ('txn/confirm', 'txn/cancel') THEN
        SELECT run_seq INTO seed_seq FROM run_event_log
        WHERE run_id = NEW.run_id AND event_type = 'run/end-seed' ORDER BY run_seq LIMIT 1;
        IF seed_seq IS NOT NULL AND EXISTS (
            SELECT 1 FROM run_event_log
            WHERE run_id = NEW.run_id AND event_type = 'txn/try' AND scope_id = NEW.scope_id AND run_seq < seed_seq
        ) THEN
...
CREATE TRIGGER run_event_log_ownership BEFORE INSERT ON run_event_log FOR EACH ROW EXECUTE FUNCTION check_event_ownership();
CREATE TRIGGER run_event_log_cancel_guard BEFORE INSERT ON run_event_log FOR EACH ROW EXECUTE FUNCTION check_no_cancel_after_pivot();
CREATE TRIGGER run_event_log_txn_projection AFTER INSERT ON run_event_log FOR EACH ROW EXECUTE FUNCTION apply_txn_projection();
...
BEGIN
    ...
    FOR item IN SELECT value FROM jsonb_array_elements(p_events) LOOP
        ...
        INSERT INTO run_event_log (run_id, run_seq, event_type, vertex_id, parent_refs, planner_id, scope_id, pin_version, ignorable, payload)
        VALUES (
        ...
        run_seq := allocated_seq; RETURN NEXT;
    END LOOP;
END;
$$;
...
REVOKE ALL ON run_event_log, run, txn_scope, txn_bracket, work_queue FROM PUBLIC;
REVOKE ALL ON FUNCTION create_run(UUID), append_events(UUID, JSONB), copy_inherited_events(UUID, JSONB) FROM PUBLIC;
GRANT USAGE ON SCHEMA public TO engine_role, coordinator_role;
GRANT SELECT ON run_event_log, run, txn_scope, txn_bracket, work_queue TO engine_role, coordinator_role;
